// Super Mario prototype — sprite-based PNG assets
import { canvas, input, audio, hud, draw, createGame, stateMachine,
         assets, camera, tilemap, spriteSheet,
         body, applyGravity, move, aabb,
...
export function start(canvasEl) {
  assets.clear();
  assets.add('mario',      './assets/mario.png');
  assets.add('goomba',     './assets/goomba.png');
  assets.add('mario_coin', './assets/mario_coin.png');
  assets.add('mario_tiles','./assets/mario_tiles.png');

  const best = savedSignal('mario_best', 0);

  // sprite sheets — created after preload
  let marioSheet, marioAnims, coinSheet, coinAnim, tileSheet;
  ...
  function initSheets() {
    marioSheet = spriteSheet(assets.getImage('mario'), 16, 16);
    marioAnims = {
    ...
    coinSheet = spriteSheet(assets.getImage('mario_coin'), 8, 12);
    coinAnim  = coinSheet.anim('spin', [0,1,2,3], 8);

    tileSheet = spriteSheet(assets.getImage('mario_tiles'), 16, 16);
  }
  ...
  function resetGame() {
    ...
    const gs = spriteSheet(assets.getImage('goomba'), 14, 14);
    enemies = ENEMY_SPAWNS.map(sp => {
      ...
      e.walkAnim = gs.anim('walk', [0,1], 5);
      e.deadAnim = gs.anim('dead', [2], 4).setLoop(false);
      e.anim = e.walkAnim;
      return e;
    });
          ...
          update(dt) {
            ...
            for (const e of enemies) {
              ...
              if (aabb(player,e)) {
                ...
                if (stomp) {
                  e.dead=true; e.dieTimer=0.5; e.anim=e.deadAnim;
  ...
  return createGame(canvasEl, {
    ...
    preload: async () => { await assets.load(); initSheets(); },
    update(dt)  { fsm ??= stateMachine(stateObj, 'menu'); fsm.update(dt); },
